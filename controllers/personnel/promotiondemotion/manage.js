...
const saveOrUpdatePromotion = async (req, res) => {
    ...
    try {
        ...
        const { rowCount: levelCount } = await pg.query({
            text: `SELECT * FROM divine."level" WHERE "id" = $1`,
            values: [level]
        });
    ...
    try {
        if (id) {
            ...
            const { rowCount } = await pg.query({
                text: `UPDATE divine."promotiondemotion" 
            ...
            // Save new promotion
            await pg.query({
                text: `INSERT INTO divine."promotiondemotion" ("userid", "title", "level", "type", "dateadded", "createdby", "status") 
