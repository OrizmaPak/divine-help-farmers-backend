...
const saveOrUpdatePropertyProduct = async (req, res) => {
    const user = req.user;
    const { id, product, member, useraccount, registrationcharge, productofficer, currency } = req.body;
        ...
        if (id) {
            ...
            const updateQuery = {
                text: `UPDATE divine."propertyproduct" 
                       SET product = $1, member = $2, useraccount = $3, registrationcharge = $4, productofficer = $5, currency = $6, status = 'ACTIVE'
                       WHERE id = $7`,
                values: [product, member, useraccount || 1, registrationcharge, productofficer, currency || 'NGN', id]
            };
            ...
            const insertQuery = {
                text: `INSERT INTO divine."propertyproduct" (product, member, useraccount, registrationcharge, createdby, productofficer, currency, dateadded, status) 
                       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), 'ACTIVE')`,
                values: [product, member, useraccount || 1, registrationcharge, user.id, productofficer, currency || 'NGN']
            };
