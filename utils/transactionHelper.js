...
const { sendSmsDnd, formatPhoneNumber } = require('./sendSms');
...
const saveTransaction = async (client, res, transactionData, req) => {
    try {
        ...
        await client.query(
            ...
            [accountnumber, credit, debit, newReference, description, ttype, status, transactiondate, req.body.whichaccount, finalValuedate, transactiondesc, createdBy, currency, userid, tfrom, transactionref??req.body.transactionref??'', cashref, req.body.branch]
        );

        // Extract the phone number from the personal account number
        const personalAccountPrefix = req.orgSettings.personal_account_prefix;
        const personalAccountNumber = req.body.personalaccountnumber;
...
const generateNewReference = async (client, accountnumber, req) => {
    ...
    // Fetch orgSettings if not present in the request
    if (!req.orgSettings) {
        ...
        req.orgSettings = orgSettingsResult.rows[0];
    }
