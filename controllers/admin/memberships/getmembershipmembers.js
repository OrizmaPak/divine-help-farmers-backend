    ...
    try {
        let query = {
            text: `SELECT m.*, 
                   ...
                   LEFT JOIN divine."User" u ON m.userid = u.id
                   LEFT JOIN divine."Position" p ON m.member = p.member AND m.userid = p.userid
                   LEFT JOIN divine."Branch" ub ON u.branch = ub.id
                   LEFT JOIN divine."Branch" pb ON p.branch = pb.id`,
            values: []
        };
        ...
        let memberships;
        console.log('Branches:', result.rows.map(row => ({ branch: row.user?.branch, id: row.user?.id, name: row.user?.firstname })));
        ...
        const countQuery = {
            text: `SELECT COUNT(*) FROM divine."Membership" m ${whereClause}`,
