...
const createOrUpdateAccount = async (req, res) => {
    const { id, accounttype, groupname, description, createdby } = req.body;
    const user = req.user;

    try {
        // Fetch organisation settings
        const { rows: orgSettingsRows } = await pg.query(`SELECT * FROM divine."Organisationsettings" WHERE id = 1`);
        ...
        let accountnumber;
        const { rows: accountRows } = await pg.query(`SELECT accountnumber FROM divine."Accounts" WHERE accountnumber LIKE '${prefix}%' ORDER BY accountnumber DESC LIMIT 1`);
        ...
        const { rows: existingAccountRows } = await pg.query(`SELECT * FROM divine."Accounts" WHERE accountnumber = $1`, [accountnumber]);
        ...
        if (id) {
            ...
            await pg.query(`UPDATE divine."Accounts" SET accountnumber = $1, groupname = $2, accounttype = $3, description = $4, dateadded = $5, createdby = $6 WHERE id = $7`, 
                [accountnumber, groupname, accounttype, description, new Date(), createdby, id]);
            await activityMiddleware(req, user.id, 'Account updated successfully', 'ACCOUNT');
        } else {
            // Create new account
            await pg.query(`INSERT INTO divine."Accounts" (accountnumber, groupname, accounttype, description, status, dateadded, createdby) VALUES ($1, $2, $3, $4, 'ACTIVE', $5, $6)`, 
                [accountnumber, groupname, accounttype, description, new Date(), createdby]);
