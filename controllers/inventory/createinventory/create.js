    ...
    try {
        for (let i = 1; i <= rowsize; i++) {
            ...
            for (let br of branches) {
                ...
                // const { rows: branchExists } = await pg.query(`SELECT * FROM divine."Branch" WHERE id = $1`, [br]);
                ...
                const { rows: itemExists } = await pg.query(`SELECT * FROM divine."Inventory" WHERE itemname = $1 AND branch = $2`, [itemname, br]);
            ...
            for (let i = 0; i < departments.length; i++) {
                ...
                await pg.query(`INSERT INTO divine."Inventory" (itemid, itemname, department, branch, units, cost, applyto, itemclass, composite, price, pricetwo, beginbalance, qty, minimumbalance, "group", description, imageone, imagetwo, imagethree, status, "reference", transactiondate, transactiondesc, reorderlevel, dateadded, createdby) 
