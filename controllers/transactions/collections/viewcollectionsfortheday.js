    ...
    const viewCollectionsForTheDay = async (req, res) => {
        ...
        try {
            ...
            const { rows: orgSettings } = await pg.query(`SELECT default_cash_account FROM divine."Organisationsettings"`);
                ...
                for (let transaction of transactionDetails) {
                    ...
                    if (whichaccount === 'PERSONAL') {
                        ...
                        if (savings.length > 0) {
                            ...
                            const { rows: users } = await pg.query(`SELECT firstname, lastname, othernames FROM divine."User" WHERE id = $1`, [savingsUserId]);
                            if (users.length > 0) {
                                const { firstname, lastname, othernames } = users[0];
                                accountName = `${firstname} ${lastname} ${othernames}`.trim();
                            }
                        ...
                        if (loans.length > 0) {
                            ...
                            const { rows: users } = await pg.query(`SELECT firstname, lastname, othernames FROM divine."User" WHERE id = $1`, [loanUserId]);
                            if (users.length > 0) {
                                const { firstname, lastname, othernames } = users[0];
                                accountName = `${firstname} ${lastname} ${othernames}`.trim();
                            }
