...
const inrequisition = async (req, res) => {
    ...
    const { rows: branchFromRows } = await pg.query(`SELECT * FROM divine."Branch" WHERE id = $1`, [branchfrom]);
    ...
    const { rows: branchToRows } = await pg.query(`SELECT * FROM divine."Branch" WHERE id = $1`, [branchto]);
    ...
    const { rows: departmentFromRows } = await pg.query(`SELECT * FROM divine."Department" WHERE id = $1 AND branch = $2`, [departmentfrom, branchfrom]);
    ...
    const { rows: departmentToRows } = await pg.query(`SELECT * FROM divine."Department" WHERE id = $1 AND branch = $2`, [departmentto, branchto]);
    ...
    try {
        ...
        for (let i = 0; i < itemids.length; i++) {
            ...
            const { rows: fallbackRowsFrom } = await pg.query(`SELECT * FROM divine."Inventory" WHERE itemid = $1 AND branch = $2 AND department = $3 ORDER BY id DESC LIMIT 1`, [itemids[i], branchfrom, departmentfrom]);
        ...
        const title = `Pending Out Requisition from ${departmentToName} to ${departmentFromName}`;
        const notificationDescription = `Your department has a pending requisition of items: ${itemNames.join(', ')} from department ${departmentToName} in branch ${branchto}.`;

        await pg.query(`INSERT INTO divine."notification" (department, branch, title, description, location, createdby) VALUES ($1, $2, $3, $4, $5, $6)`, [departmentfrom, branchfrom, title, notificationDescription, 'approveinrequisition', req.user.id]);
