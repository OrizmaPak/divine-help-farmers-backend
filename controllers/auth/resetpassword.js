...
const resetPassword = async (req, res) => {
  try {
    ...
    const { rows: [user] } = await pg.query(`SELECT * FROM divine."VerificationToken" WHERE token = $1`, [passwordtoken]);
    if (!user) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        status: false,
        message: "Expired token",
    ...
    await pg.query(`UPDATE divine."User" SET password = $1 WHERE email = $2`, [hashpwd, email]);
    
    // Remove the verification token
    await pg.query(`DELETE FROM divine."VerificationToken" WHERE token = $1`, [passwordtoken]);
