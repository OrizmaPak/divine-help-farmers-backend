...
async function generateSentenceControllerwithoutdate(code) {
  try {
    // Validate that the code is provided
    if (!code || typeof code !== "string") {
    ...
    // Check if the code already exists in the codeandmeaningforlanding table
    const { rows: [existingEntry] } = await pg.query({
      text: `SELECT meaning FROM divine."codeandmeaningforlanding" WHERE code = $1`,
      values: [code]
    });
    if (existingEntry) {
      return existingEntry.meaning;
    }
    ...
    Must be within D1T (1st day) to D31T (last day).
    Cannot exceed this range.
    Week Occurrences (WO1 to WO5):
    ...
    Ensure that combined intervals do not conflict or create ambiguous schedules.
    Clearly define each combination to maintain unambiguous.
    Extensibility:
...
You are a sentence Generator. Using the provided instructions, convert the following date code into a sentence.

 Date Code: "${code}"
 let the return sentence always start with "Meetings hold every"

Sentence: Meetings hold every `;

    // Generate the date code using OpenAI
    let dateCode = await generateText(prompt);

    // Ensure the sentence starts with "Meetings hold every"
    if (!dateCode.startsWith("Meetings hold every")) {
      dateCode = "Meetings hold every " + dateCode;
    }

    // Replace "31st day of the month" with "last day of the month"
    dateCode = dateCode.replace("31st day of the month", "last day of the month");
    ...
    // Save the generated sentence to the database
     try {
      // Check if the code already exists in the codeandmeaningforlanding table
      const { rows: [existingEntry] } = await pg.query({
        text: `SELECT 1 FROM divine."codeandmeaningforlanding" WHERE code = $1`,
        values: [code]
      });

      if (!existingEntry) {
        const insertQuery = {
          text: `INSERT INTO divine."codeandmeaningforlanding" (code, meaning, createdby, dateadded, status) VALUES ($1, $2, $3, NOW(), 'ACTIVE')`,
          values: [code, dateCode, 0]
        };
        await pg.query(insertQuery);
      } 
    } catch (error) {
      console.error("Error inserting into codeandmeaningforlanding:", error);
    ...
    console.error("Error in generateSentenceControllerwithoutdate:", error);
    return 'Meetings hold every AI failed to respond';
  }
}
...
module.exports = {
  ...
  generateSentenceControllerwithoutdate
};
