...
const getSuspensions = async (req, res) => {
    ...
    try {
        ...
        if (req.query.q) {
            // Fetch column names from the 'suspension' table
        ...
        const suspensions = result.rows;

        // Get total count for pagination
        const countQuery = {
            text: `SELECT COUNT(*) FROM divine."suspension" ${whereClause}`,
            values: query.values.slice(0, -2) // Exclude limit and offset
        };
        const { rows: [{ count: total }] } = await pg.query(countQuery);
        const pages = divideAndRoundUp(total, limit);

        await activityMiddleware(req, user.id, 'Suspensions fetched successfully', 'SUSPENSION');

        return res.status(StatusCodes.OK).json({
            status: true,
            message: "Suspensions fetched successfully",
        ...
        await activityMiddleware(req, user.id, 'An unexpected error occurred fetching suspensions', 'SUSPENSION');
