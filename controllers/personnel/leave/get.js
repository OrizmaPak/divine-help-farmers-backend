    ...
    try {
        let query = {
            text: `
                ...
                FROM divine."leave" l
                JOIN divine."User" u ON l.userid = u.id
        ...
        if (req.query.startdate) {
            whereClause += ` AND "startdate" >= $${valueIndex}`;
            query.values.push(req.query.startdate);
            valueIndex++;
        }

        if (req.query.enddate) {
            whereClause += ` AND "enddate" <= $${valueIndex}`;
        ...
        if (req.query.q) {
            // Fetch column names from the 'leave' table
        ...
        const countQuery = {
            text: `SELECT COUNT(*) FROM divine."leave" WHERE status = 'ACTIVE' ${whereClause}`,
            values: query.values.slice(0, -2) // Exclude limit and offset
        };
        const { rows: [{ count: total }] } = await pg.query(countQuery);
        const pages = divideAndRoundUp(total, limit);

        await activityMiddleware(req, user.id, 'Leaves fetched successfully', 'LEAVE');
        ...
        await activityMiddleware(req, user.id, 'An unexpected error occurred fetching leaves', 'LEAVE');
