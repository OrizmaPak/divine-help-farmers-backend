...
const saveTransactionMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (credit > 0 && debit > 0) {
            await saveFailedTransaction(client, req, res, 'Invalid transaction', generateNewReference(client, accountnumber, req, res), whichaccount);
        ...
        if (accountResult.rowCount === 0 && !accountnumber.startsWith(orgSettings.personal_account_prefix)) {
            await saveFailedTransaction(client, req, res, 'Invalid account number', generateNewReference(client, accountnumber, req, res), whichaccount);
            await client.query('COMMIT'); // Commit the transaction
        ...
        if (!orgSettings.allow_back_dated_transaction && new Date(transactiondate) < currentDate) {
            ...
            await saveFailedTransaction(client, req, res, reasonForRejection, generateNewReference(client, accountnumber, req, res), whichaccount);
            await client.query('COMMIT'); // Commit the transaction
        ...
        if (whichaccount === 'SAVINGS') {            
            ...
            req.body['userid'] = accountuser.id;
       

            const account = accountResult.rows[0];
            if (account.status !== 'ACTIVE') {
                await saveFailedTransaction(client, req, res, 'Account is not active.', generateNewReference(client, accountnumber, req, res), whichaccount);
                await client.query('COMMIT'); // Commit the transaction
            ...
            if (savingsProductResult.rowCount === 0) {
                await saveFailedTransaction(client, req, res, 'Invalid savings product', generateNewReference(client, accountnumber, req, res), whichaccount);
                await client.query('COMMIT'); // Commit the transaction
            ...
            if (savingsProduct.status !== 'ACTIVE') {
                await saveFailedTransaction(client, req, res, 'Inactive savings product', generateNewReference(client, accountnumber, req, res), whichaccount);
                await client.query('COMMIT'); // Commit the transaction
            ...
            await applyMinimumCreditAmountPenalty(client, req, res, orgSettings);
            ...
            await savingsDebit(client, req, res, next, accountnumber, debit, description, ttype, transactionStatus, savingsProduct, whichaccount);
