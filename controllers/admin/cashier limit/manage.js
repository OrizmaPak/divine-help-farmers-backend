...
const manageCashierLimit = async (req, res) => {
    try {
        ...
        const { rows: [userExists] } = await pg.query(`SELECT * FROM divine."User" WHERE id = $1`, [cashier]);
        if (!userExists) {
        ...
        if (id) {
            ...
            if (status) {
                const { rows: [updatedCashierLimit] } = await pg.query(`UPDATE divine."Cashierlimit" SET status = $1 WHERE id = $2 RETURNING *`, [status, id]);
                ...
                const { rows: [updatedCashierLimit] } = await pg.query(`UPDATE divine."Cashierlimit" SET depositlimit = $1, withdrawallimit = $2 WHERE id = $3 RETURNING *`, [depositlimit, withdrawallimit, id]);
                if (!updatedCashierLimit) {
                    return res.status(StatusCodes.NOT_FOUND).json({
                        status: false,
                        message: "Cashier limit not found",
