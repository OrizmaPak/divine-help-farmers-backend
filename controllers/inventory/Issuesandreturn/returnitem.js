...
const updateReturnItem = async (req, res) => {
    ...
    try {
        ...
        if (supplier) {
            ...
            const { rows: [supplierExists] } = await pg.query(`SELECT * FROM divine."Supplier" WHERE id = $1`, [supplier]);
            if (!supplierExists) {
            ...
            const { rows: [staffExists] } = await pg.query(`SELECT * FROM divine."User" WHERE id = $1`, [staff]);
            if (!staffExists) {
