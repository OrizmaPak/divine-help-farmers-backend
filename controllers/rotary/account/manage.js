    ...
    const createOrUpdateRotaryAccount = async (req, res) => {
        ...
        try {
            ...
            const scheduleQuery = {
                text: `SELECT * FROM divine."rotaryschedule" WHERE accountnumber = $1 AND currentschedule = 'YES'`,
            ...
            if (existingSchedules.length > 0) {
                ...
                const deleteScheduleQuery = {
                    text: `DELETE FROM divine."rotaryschedule" 
            ...
            if (poolNumber === 'SEQUENCE') {
                ...
                const maxScheduleQuery = {
                    text: `SELECT MAX(duedate) as maxduedate FROM divine."rotaryschedule" WHERE accountnumber = $1`,
                    values: [generatedAccountNumber]
                };
                const { rows: [{ maxduedate }] } = await pg.query(maxScheduleQuery);
                const totalAmountQuery = {
                    text: `SELECT SUM(amount) as totalamount FROM divine."rotaryschedule" WHERE accountnumber = $1`,
                    values: [generatedAccountNumber]
                };
                ...
                const randomScheduleQuery = {
                    text: `SELECT duedate FROM divine."rotaryschedule" WHERE accountnumber = $1 ORDER BY RANDOM() LIMIT 1`,
                    values: [generatedAccountNumber]
                };
