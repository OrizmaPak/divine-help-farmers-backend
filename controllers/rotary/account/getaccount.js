    ...
    const getAccountsAndSchedules = async (req, res) => {
        ...
        try {
            ...
            let query = {
                text: `SELECT * FROM divine."rotaryaccount"`,
                values: []
            };
            ...
            const countQuery = {
                text: `SELECT COUNT(*) FROM divine."rotaryaccount" ${whereClause}`,
            ...
            const processedAccounts = await Promise.all(accounts.map(async (account) => {
                ...
                const registrationPointQuery = {
                    text: `SELECT registrationpoint AS name FROM divine."Registrationpoint" WHERE id = $1`,
                ...
                // Fetch product name
                const productQuery = {
                    text: `SELECT product AS productname FROM divine."rotaryProduct" WHERE id = $1`,
                    values: [productId]
                };
                ...
                // Fetch member name
                const memberQuery = {
                    text: `SELECT member AS membername FROM divine."DefineMember" WHERE id = $1`,
                    values: [memberId]
                };
                ...
                // Fetch branch name
                const branchQuery = {
                    text: `SELECT branch AS branchname FROM divine."Branch" WHERE id = $1`,
                    values: [branchId]
                };
