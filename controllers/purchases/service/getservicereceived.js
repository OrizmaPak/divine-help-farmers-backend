...
const getServicesReceived = async (req, res) => {
    const user = req.user;

    try {
        let query = {
            text: `SELECT * FROM divine."Service" WHERE status = 'ACTIVE'`,
            values: []
        };
        ...
        for (const ref in groupedServices) {
            ...
            const branchQuery = `SELECT branch FROM divine."Branch" WHERE id = $1`;
            const supplierQuery = `SELECT supplier FROM divine."Supplier" WHERE id = $1`;
...
module.exports = { getServicesReceived };
