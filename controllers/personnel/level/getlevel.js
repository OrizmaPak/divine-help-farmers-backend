...
const getLevel = async (req, res) => {
    ...
    try {
        let query = {
            text: `SELECT * FROM divine."level" WHERE status = 'ACTIVE'`,
            values: []
        };
        ...
        for (let level of levels) {
            ...
            const allowancesResult = await pg.query(
                `SELECT * FROM divine."allowances" WHERE level = $1`,
                [levelId]
            );
            level.allowances = allowancesResult.rows;

            const deductionsResult = await pg.query(
                `SELECT * FROM divine."deductions" WHERE level = $1`,
                [levelId]
            );
        ...
        const countQuery = {
            text: `SELECT COUNT(*) FROM divine."level" ${whereClause}`,
