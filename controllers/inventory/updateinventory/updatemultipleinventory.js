...
const getFallbackData = async (itemid, department) => {
    ...
    const fallbackResult = await pg.query(
        `SELECT * FROM divine."Inventory" WHERE itemid = $1 AND department = $2 ORDER BY id DESC LIMIT 1`, 
        [itemid, department]
    );
        ...
        for (const [index, item] of items.entries()) {
            ...
            if (!department) {
                const { rows: itemDepartments } = await pg.query(
                    `SELECT department FROM divine."Inventory" WHERE itemid = $1 GROUP BY department`,
                    [itemid]
                );
            ...
            if (!branch) {
                for (let dept of departments) {
                    ...
                    const { rows } = await pg.query(
                        `SELECT branch FROM divine."Department" WHERE id = $1`,
                        [dept]
                    );
