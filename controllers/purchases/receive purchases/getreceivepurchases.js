...
const getReceivePurchases = async (req, res) => {
    ...
    try {
        let query = {
            text: `SELECT reference, MAX(id) as id, MAX(transactiondate) as transactiondate, MAX(supplier) as supplier FROM divine."Inventory" WHERE reference LIKE 'RP-%' GROUP BY reference`,
            values: []
        };
