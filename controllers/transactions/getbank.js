...
const getBankTransactions = async (req, res) => {
    ...
    try {
        let query = {
            text: `SELECT * FROM divine."banktransaction"`,
            values: []
        };
        ...
        if (req.query.q) {
            // Fetch column names from the 'banktransaction' table
        ...
        const bankTransactions = result.rows;

        // Get total count for pagination
        const countQuery = {
            text: `SELECT COUNT(*) FROM divine."banktransaction" ${whereClause}`,
            values: query.values.slice(0, -2) // Exclude limit and offset
        };
        const { rows: [{ count: total }] } = await pg.query(countQuery);
        const pages = divideAndRoundUp(total, limit);

        await activityMiddleware(req, user.id, 'Bank transactions fetched successfully', 'BANK_TRANSACTION');
        ...
        await activityMiddleware(req, user.id, 'An unexpected error occurred fetching bank transactions', 'BANK_TRANSACTION');
