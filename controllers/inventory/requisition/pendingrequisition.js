...
const getpendingrequisition = async (req, res) => {
    ...
    const status = 'PENDING REQUISITION'; // Set status hardcoded to 'PENDING REQUISITION'
    ...
    try {
        ...
        for (const item of inventory) {
            ...
            if (item.qty < 0) {
                ...
                const { rows: [{ branch: branchfromname }] } = await pg.query(`SELECT branch FROM divine."Branch" WHERE id = $1`, [item.branch]);
                const { rows: [{ department: departmentfromname }] } = await pg.query(`SELECT department FROM divine."Department" WHERE id = $1`, [item.department]);
                ...
                // Fetch branch and department names
                const { rows: [{ branch: branchtoname }] } = await pg.query(`SELECT branch FROM divine."Branch" WHERE id = $1`, [item.branch]);
                const { rows: [{ department: departmenttoname }] } = await pg.query(`SELECT department FROM divine."Department" WHERE id = $1`, [item.department]);
