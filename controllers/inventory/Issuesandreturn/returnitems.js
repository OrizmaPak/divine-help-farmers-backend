...
const managereturnitems = async (req, res) => {
    ...
    const supplierExists = await pg.query(`SELECT * FROM divine."Supplier" WHERE id = $1`, [itemDetails.supplier]);
    ...
    try {
        ...
        for (let i = 0; i < rowsize; i++) {
            ...
            const fallbackData = {
                ...
                qty: -qtys[i],
                status: "RETURNED",
                transactiondesc: "Returned item(s)",
                dateadded: new Date(),
            ...
            if (reference) {
                await pg.query(`DELETE FROM divine."Inventory" WHERE reference = $1`, [reference]);
            }

            // Insert the data into the Inventory table
            await pg.query(`INSERT INTO divine."Inventory" (itemid, branch, department, qty, status, transactiondesc, dateadded, createdby, itemname, units, cost, price, pricetwo, beginbalance, minimumbalance, "group", applyto, itemclass, composite, compositeid, description, imageone, imagetwo, imagethree, sellingprice, reference, transactiondate, issue, issuetype, supplier) 
