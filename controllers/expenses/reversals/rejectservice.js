...
const rejectService = async (req, res) => {
    ...
    try {
        ...
        const serviceQuery = `SELECT * FROM divine."Service" WHERE id = $1`;
        ...
        let staffphone;
        if (staff) {
            const staffQuery = `SELECT * FROM divine."User" WHERE id = $1`;
        ...
        const { rows: [organisationSettings] } = await pg.query(`SELECT * FROM divine."Organisationsettings"`);
        ...
        const insertQuery = `
            INSERT INTO divine."Service" (serviceid, supplier, staff, servicetype, description, amount, amountfrom, amountto, servicestartdate, serviceenddate, branch, dateadded, createdby, issue, reference, status)
