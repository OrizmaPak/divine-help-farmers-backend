...
const organizationsettings = async (req, res) => {
  try {
    ...
    const {
      ...
      new_member_registration_charge = null,
    ...
    const orgData = {
      ...
      new_member_registration_charge: parseNumber(new_member_registration_charge),
    ...
    // Database operation
    const { rows: existing } = await pg.query('SELECT * FROM divine."Organisationsettings"');
    ...
    if (existing.length === 0) {
      ...
      const { rows } = await pg.query(
        `INSERT INTO divine."Organisationsettings" (${columns}) 
      ...
      await pg.query(
        `UPDATE divine."Organisationsettings" 
