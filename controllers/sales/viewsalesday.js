    ...
    try {
        ...
        let inventoryQuery = {
            text: `
                ...
                JOIN divine."transaction" T ON I."reference" = T."transactionref"
                JOIN divine."User" U ON T."userid" = U."id"
        ...
        if (branch) {
            ...
            const branchRes = await pg.query({
                text: `
                    ...
                    JOIN divine."User" U ON B.id = U.branch
                    WHERE U.id = $1
                `,
                values: [userid]
            });
        ...
        const transactionPromises = Object.keys(groupedInventory).map(async (reference) => {
            try {
                ...
                const transactionRes = await pg.query({
                    text: `SELECT * FROM divine."transaction" WHERE transactionref = $1`,
                    values: [reference]
                });
